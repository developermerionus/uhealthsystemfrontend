		...
		<view class="body-wrap">
			<view class="form-wrap">
				...
				<view class="input-wrap" v-show="registerMode == 'account'">
					<view class="content"><input type="text" :placeholder="$lang('accountPlaceholder')" placeholder-class="input-placeholder" class="input" v-model="formData.account" /></view>
				</view>
				<view class="input-wrap" v-show="registerMode == 'account'">
					<view class="content">
						<view class="content"><input type="password" :placeholder="$lang('passwordPlaceholder')" placeholder-class="input-placeholder" class="input" v-model="formData.password" /></view>
					</view>
				</view>
				<view class="input-wrap" v-show="registerMode == 'account'">
					<view class="content">
						<input type="password" :placeholder="$lang('passwordPlaceholder')" placeholder-class="input-placeholder" class="input" v-model="formData.rePassword" />
					</view>
				</view>
				<view class="input-wrap">
					<view class="content">
						<input type="text" :placeholder="$lang('captchaPlaceholder')" placeholder-class="input-placeholder" class="input" v-model="formData.vercode" />
			...
			<view class="btn_view"><button type="primary" @click="register" class="login-btn color-base-border color-base-bg">{{$lang('registered')}}</button></view>
			<view class="regisiter-agreement">
				{{$lang('registerTips')}}
...
export default {
	...
	data() {
		return {
			...
			item: {superiors: 'suzh0125', branch: '左区-left'},
			countryList: [
				{
					"id":172,
					"name":"China"
	...
	methods: {
		selectCountry() {
			uni.showActionSheet({
				itemList: this.countryList.map((item)=>{return item.name}),
				success: (res)=>{
					// console.log(res);
		...
		verify(data) {
			...
			// 用户名注册
			if (this.registerMode == 'account') {
				var rule = [{ name: 'username', checkType: 'required', errorMsg: '请输入账号' }, 
				{ name: 'password', checkType: 'required', errorMsg: '请输入密码' },
				{ name: 'marketregion', checkType: 'required', errorMsg: '请选择全球市场' },
				],
