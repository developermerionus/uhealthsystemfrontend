			...
			<view class="goods-container">
						...
						<swiper class="swiper" @change="swiperChange" :interval="swiperInterval" :autoplay="swiperAutoplay">
							<swiper-item v-for="(item, index) in goodsSkuDetail.sku_images" :key="index" :item-id="'goods_id_' + index">
								<view class="item" @click="previewMedia(index)">
									<image :src="$util.img(item, { size: 'big' })" @error="swiperImageError(index)" mode="center" />
								</view>
							</swiper-item>
						</swiper>
				...
				<view class="group-wrap padding-top">
					...
					<view class="goods-module-wrap info">
						...
						<view class="member-card-wrap" @click="$util.redirectTo('/otherpages/member/card_buy/card_buy')" v-if="membercard">
							...
							<view class="info">开通{{ membercard.level_name }}<text v-if="memberCardDiscount > 0">预计可省{{ memberCardDiscount }}元</text><text v-else>省更多！</text></view>
					...
					<view class="goods-evaluate" @click="toEvaluateDetail(goodsSkuDetail.goods_id)">
						<view class="tit" :class="{ active: goodsEvaluate.content }">
							<view>
								...
								<text class="evaluate-item-empty" v-if="!goodsEvaluate.content">{{$lang('common.no_review')}}</text>
				...
				<view class="goods-detail-tab">
					<view class="detail-tab">
						<view class="tab-item" :class="detailTab == 0 ? 'active' : ''" @click="detailTab = 0">{{$lang('common.product_detail')}}</view>
					...
					<uni-popup ref="posterPopup" type="bottom" class="poster-layer">
						<template v-if="poster != '-1'">
							<view :style="{ height: posterHeight > 0 ? posterHeight + 80 + 'px' : '' }">
								<view class="image-wrap"><image :src="$util.img(poster)" :show-menu-by-longpress="true" :style="{ height: posterHeight > 0 ? posterHeight + 'px' : '' }" /></view>
								<!-- #ifdef MP || APP-PLUS  -->
			...
			<template v-if="goodsSkuDetail.goods_state == 1">
				...
				<block v-else>
					<ns-goods-action-button
						...
						@click="joinCart"
						v-if="goodsSkuDetail.is_virtual == 0 && !goodsSkuDetail.goods_name.includes('套餐')"
					/>
