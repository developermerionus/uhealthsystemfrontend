	...
	<view class="order-container" :class="{ 'safe-area': isIphoneX }" :data-theme="themeStyle">
		...
		<view class="site-wrap">
			...
			<view class="site-footer">
				...
				<view class="order-cell">
					...
					<view class="box"><input type="text" :placeholder="$lang('common.remark')" placeholder-class="color-tip" v-model="orderCreateData.buyer_message" /></view>
				</view>
				<view class="order-cell clear-flex" v-if="orderPaymentData.recommend_member_card">
					...
					<view class="box member-card-wrap">
						<view class="head" @click="selectMemberCard">
							...
							<view class="info">开通{{ orderPaymentData.recommend_member_card.level_name }}<text>本单预计可省{{ orderPaymentData.recommend_member_card.discount_money|moneyFormat }}元</text></view>
							<text class="iconfont" :class="orderPaymentData.is_open_card == 1 ? 'iconyuan_checked color-base-text' : 'iconyuan_checkbox'"></text>
						</view>
						<view class="body" v-if="orderCreateData.is_open_card">
							<view class="item" :class="{'active color-base-border': item.key == orderPaymentData.member_card_unit }" v-for="(item, index) in cardChargeType" :key="index" @click="selectMembercardUnit(item.key)">
								<view class="title">{{ item.title }}</view>
								<view class="price">{{ $lang('common.currencySymbol') }}{{ parseFloat(item.value) }}/{{ item.unit }}</view>
		...
		<view class="order-submit" :class="{ 'bottom-safe-area': isIphoneX }">
			<view class="order-settlement-info">
				<text class="font-size-base color-tip margin-right">{{$lang('common.total')}}{{ orderPaymentData.goods_num }}{{$lang('common.num_of_buy')}}</text>
			...
			<view class="submit-btn"><button type="primary" size="mini" @click="openChoosePayment()">{{$lang('common.submit_payment')}}</button></view>
		</view>
					...
					<view>
						...
						<block v-if="orderCreateData.is_invoice == 1">
							<view class="invoice-cell">
								...
								<view class="option-grpup">
									<view class="option-item" :class="{ 'color-base-bg active': orderCreateData.invoice_type == 1 }" @click="changeInvoiceType(1)">纸质</view>
									<view class="option-item" :class="{ 'color-base-bg active': orderCreateData.invoice_type == 2 }" @click="changeInvoiceType(2)">电子</view>
								</view>
							</view>
							<view class="invoice-cell">
						...
						<block v-if="manjian.length">
							<view class="order-cell" v-for="(item, index) in manjian" :key="index">
								...
								<view class="box" v-if="item.discount_array.real_discount_money > 0">
									<text class="unit color-base-text">- {{ $lang('common.currencySymbol') }}</text>
									<text class="money color-base-text">{{ item.discount_array.real_discount_money | moneyFormat }}</text>
								</view>
							</view>
						</block>
