<template>
	<view class="detail-container" :class="{ 'safe-area': isIphoneX }" :data-theme="themeStyle">
					...
					<view class="goods-info">
						...
						<view class="goods-action">
							<block v-if="orderData.is_enable_refund">
								<view hover-class="none" @click="goRefund(orderData.order_goods_id)" v-if="orderData.refund_status == 0">
									<view class="order-box-btn">退款</view>
								</view>
								<view @click="goRefundDetail(orderData.order_goods_id)" v-else><view class="order-box-btn">查看退款</view></view>
							</block>
						</view>
					</view>
				</view>
		...
		<view class="order-money">
			...
			<view class="order-cell" v-if="orderData.price > 0">
				<view class="box align-right">
					<text>实付金额：</text>
					<text class="color-base-text">
		...
		<!-- 选择支付方式弹窗 -->
		<ns-payment ref="choosePaymentPopup" :payMoney="orderData.price" @confirm="orderPay(orderData)"></ns-payment>
...
export default {
	...
	onShow() {
		...
		if (uni.getStorageSync('token')) {
			...
			this.$util.redirectTo('/pages/login/login/login', { back: '/pages/order/detail_point/detail_point?order_id=' + this.orderId });
		}
		this.getKekuConfig();
	},
	methods: {
