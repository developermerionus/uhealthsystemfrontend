				...
				<view class="order-list" v-if="orderList.length > 0">
					<view class="order-item" v-for="(orderItem, orderIndex) in orderList" :key="orderIndex">
						<view class="order-header" :class="{ waitpay: orderStatus == 'waitpay' && orderItem.order_status == 0 }">
							...
							></view>
							<text class="font-size-base">{{$lang('common.order_no')}}：{{ orderItem.order_no }}</text>
						...
						<view class="order-footer">
							<view class="order-base-info">
								<view class="total">
									<text class="font-size-sub">{{$lang('common.total')}}{{ orderItem.goods_num }}{{$lang('common.product_in_cart')}}</text>
						...
						<!-- 	<view class="order-action" v-else-if="orderItem.action.length == 0 && orderItem.is_evaluate == 1 && evaluateConfig.evaluate_status == 1">
							...
							<view class="order-action" v-else><view class="order-box-btn color-base-border color-base-text" @click="orderDetail(orderItem)">{{$lang('common.more_detail')}}</view></view>
						</view>
					</view>
				</view>
