<template>
	<view class="detail-container" :class="{ 'safe-area': isIphoneX }" :data-theme="themeStyle">
		...
		<view class="verify-code-wrap" :style="orderData.virtual_goods ? 'margin-top: -69rpx;' : ''" v-else>
			...
			<view class="hr"></view>
			<view class="code"><image :src="$util.img(orderData.virtualgoods)"></image></view>
		</view>
...
export default {
	...
	onShow() {
		...
		if (uni.getStorageSync('token')) {
			...
			this.$util.redirectTo('/pages/login/login/login', { back: '/pages/order/detail_virtual/detail_virtual?order_id=' + this.orderId });
		}
		this.getKekuConfig();
	},
	methods: {
