						...
						<view class="text-box">
							...
							<view class="other-info">
								<view class="other-info-box" v-for="(i, t) in imgList[index]" :key="t">
									...
									<view class="imgDel" @click="deleteImg(i, index)"><text class=" icon iconfont icondelete"></text></view>
								</view>
...
export default {
	...
	onLoad(options) {
		...
		if (!uni.getStorageSync('token')) {
			this.$util.redirectTo('/pages/login/login/login', { back: '/pages/order/evaluate/evaluate?order_id=' + this.orderId }, 'redirectTo');
		}
		this.getUserInfo();
