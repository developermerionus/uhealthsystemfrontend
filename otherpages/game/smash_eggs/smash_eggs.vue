		...
		<view>
			...
			<view class="eggs-wrap">
				<view class="box" @click="lottery">
					<image :src="$util.img('upload/uniapp/game/golden_eggs.png')" mode="widthFix"></image>
				</view>
				<view class="box" @click="lottery">
					<image :src="$util.img('upload/uniapp/game/golden_eggs.png')" mode="widthFix"></image>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			lottery() {
				if (!uni.getStorageSync('token')) {
					this.$refs.login.open('/otherpages/game/smash_eggs/smash_eggs?id=' + this.id);
					return;
				}
