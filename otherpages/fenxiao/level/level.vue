	...
	<view class="container" :data-theme="themeStyle">
						...
						<block v-for="(item, index) in levelList" :key="index">
							...
							<view class="level_list">
								<view class="level_list-top">
									<image :src="$util.img('upload/uniapp/fenxiao/level/v1.png')"/></image>
									<text>{{index+1}}</text>
								</view>
								<view class="font-son">{{ item.level_name }}</view>
							</view>
		...
		<view class="level-bottom" v-if="fenxiaoInfo.condition.last_level">
			<view class="level-bottom-name">
				...
				<view class="head-bg color-base-text" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/fenxiao/level/bg3.png') + ')' }">
