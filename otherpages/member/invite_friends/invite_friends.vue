					...
					<view class="invitelist">
						<view class="list-item" v-for="(item, index) in inviteList">
							<view class="img color-base-border">
								<image mode="aspectFit" :src="item.headimg == '' ? $util.img($util.getDefaultImage().default_headimg) : $util.img(item.headimg)"></image>
							</view>
				...
				<view class="desc">
					...
					<view class="desc_list">
						...
						<view class="" v-if="$util.inArray('coupon', info.type) != -1">
							<text>●</text>
							可得{{ info.coupon.split(',').length }}张优惠券</text>
						</view>
					</view>
				</view>
			</view>
...
export default {
	...
	onLoad(option) {
		...
		if (uni.getStorageSync('token')) {
			...
			setTimeout(() => {
				this.$refs.login.open('/otherpages/member/invite_friends/invite_friends');
			});
		}
	},
	onShow() {
