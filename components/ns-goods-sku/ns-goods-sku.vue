		...
		<uni-popup ref="skuPopup" type="bottom" class="sku-layer">
			<view class="sku-info" :style="{ height: systemInfo.windowHeight * 1 + 'px' }">
				<view class="header">
					...
					<view v-else class="img-wrap"><image :src="$util.img(goodsDetail.sku_image, { size: 'mid' })" @error="imageError()" mode="aspectFit" /></view>
					...
					<scroll-view scroll-y class="wrap">
						...
						<view class="number-wrap">
							<view class="number-line">
								...
								<text class="title font-size-base" v-else>{{$lang('common.buy_quantity')}}</text>
				...
				<view class="footer" @click="confirm()">
					<button type="primary" v-if="goodsDetail.stock && goodsDetail.stock != 0">{{$lang('common.confirm')}}</button>
				...
				if (this.type == 'join_cart') {
					this.$api.sendRequest({
						...
						success: res => {
							...
							if (data > 0) {
								this.$util.showToast({
									title: this.$lang('common.add_successfully')
								});
