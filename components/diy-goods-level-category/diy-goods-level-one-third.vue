						...
						<scroll-view scroll-y="true" :style="{ height: 'calc(100% - ' + height + ' )' }" @scrolltolower="getGoodsList()">
							...
							<block v-for="(item, index) in goodsList" :key="index">
								<view class="twoScroll-item-box" @click="toDetail(item.sku_id)">
									<image class="twoScroll-item-image" :src="$util.img(item.sku_image, { size: 'mid' })" @error="goodsImageError(index)" mode="aspectFill" />
		...
		getGoodsList() {
			...
			this.$api.sendRequest({
				...
				data: {
					...
					category_id: this.cateList[this.oneIndex].category_id_1
				},
				success: res => {
					this.isNetwork = 1;
