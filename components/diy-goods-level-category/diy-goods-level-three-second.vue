						...
						<view class="cate-right-box" v-for="(item, index) in twoCateList" :key="index">
							...
							<view class="cate-goods-list">
								<view class="cate-goods-item" v-for="(i, j) in item.child_list" :key="j" @click="toListDetail(i.category_id_3)">
									<image v-if="i.image" class="goods-pic" :src="$util.img(i.image)" @error="threeCateImageError(index, j)" mode="aspectFit"></image>
									<image v-else class="goods-pic" :src="$util.getDefaultImage().default_goods_img" mode="aspectFit"></image>
