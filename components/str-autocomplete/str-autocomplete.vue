...
export default {
  ...
  created() {
	...
	this.initial_city = this.importvalue;  // 手动添加变量防止创建时showToast，非插件自带
  },
  methods: {
    onInput(event) {
    ...
    filterList(stringExp) {
      ...
      if (tempArray.length === 0 && stringExp.length !== 0 && stringExp !== this.initial_city) {
        uni.showToast({
          title: '没有匹配的字符串',
          mask: false,
          icon: 'none',
    ...
    selectThisItem(item) {
      ...
      this.needShow = false;
      this.$emit('change', item.originalEle[this.label] || item.originalEle);
      this.$emit('select', item.originalEle);
    }
  }
};
</script>
