<template>
	<view class="bargain" :style="{backgroundImage:'url('+ $util.img('upload/uniapp/bargain/bargain_background1.png') +')'}"
		...
		<view class="bargin_content">
			<view class="uer_info_base">
				<view class="user_info_img">
					<image :src="info.headimg  == '' ? $util.img($util.getDefaultImage().default_headimg) : $util.img(info.headimg)"
					 mode="widthFix"></image>
				</view>
			...
			<view class="goods">
				<view class="goods_img">
					<image :src="$util.img(info.sku_image)" mode="aspectFit"></image>
				</view>
				...
				<view class="bragain_recode">
					<view class="bragain_recode_list">
						<block v-for="(item, index) in bargainRecord" :key="index">
							<image :src="item.headimg == '' ? $util.img($util.getDefaultImage().default_headimg) : $util.img(item.headimg)"
							 v-if="index < 4"></image>
						</block>
			...
			<view class="invitation_peo">
				...
				<view v-if="info.self" class="flex-box">
					...
					<block v-if="info.status == 2">
						<button class="btn" @click="$util.redirectTo('/promotionpages/bargain/list/list')">低价拿更多商品</button>
					</block>
				</view>
				<view v-else class="flex-box">
					...
					<block v-else>
						<button class="btn" @click="$util.redirectTo('/promotionpages/bargain/list/list')">我也要低价拿</button>
					</block>
				</view>
			</view>
		</view>
