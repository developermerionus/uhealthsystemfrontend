			...
			<view class="address-box" v-if="orderPaymentData.delivery.delivery_type != 'store'">
				<view class="info-wrap" v-if="orderPaymentData.member_address" @click="selectAddress">
					...
					<view class="content">
						...
						<view class="desc-wrap">{{ orderPaymentData.member_address.full_address ? orderPaymentData.member_address.full_address : '' }} {{ orderPaymentData.member_address.address ? orderPaymentData.member_address.address : '' }}</view>
					</view>
				</view>
				...
				<view class="local-box" v-if="orderPaymentData.delivery.delivery_type == 'local'">
					<view class="order-cell">
						...
						<view class="box">
							...
							<text v-if="deliveryWeek"><template v-if="timeInfo.showTime">（</template>{{ deliveryWeek }}<template v-if="timeInfo.showTime">）</template></text>
		...
		<view class="site-wrap">
			...
			<view class="site-footer">
				<view class="order-cell">
					...
					<view class="box text-overflow" @click="openSitePromotion()"><text>组合套餐</text></view>
			...
			<view class="promotion-popup popup">
				...
				<scroll-view scroll-y="true" class="popup-body" :class="{ 'safe-area': isIphoneX }">
					<view>
						<view class="order-cell">
							<text class="tit">
								<text class="promotion-mark ns-gradient-promotionpages-payment" :data-theme="themeStyle">组合套餐</text>
								{{ orderPaymentData.bunding_info.bl_name }}
							</text>
							<view class="box">
								<text class="unit color-base-text">- {{ $lang('common.currencySymbol') }}</text>
								<text class="money color-base-text">{{ promotionMoney | moneyFormat }}</text>
							</view>
						</view>
					</view>
				</scroll-view>
