	...
	<view :data-theme="themeStyle">
		<view v-show="show">
			<mescroll-uni @getData="getData" ref="mescroll" :size="10" v-if="seckillId && addonIsExit.seckill" @scroll="scroll">
				<block slot="list">
					<view class="ns-adv">
						<image :src="$util.img('upload/default/diy_view/index_seckill_gg.png')" mode="widthFix"></image>
					</view>
					...
					<view class="time-wrap">
						...
						<scroll-view :scroll-x="true" class="scroll-wrap" scroll-with-animation :scroll-into-view="'a' + seckillId">
							<view class="time-item" v-for="(item, key) in timeList" :key="key" :id="'a' + item.id" @click="ontabtap(item.id, key)">
								...
								<view class="em font-size-activity-tag color-sub" :class="seckillId == item.id ? 'color-base-bg active' : ''"
								 v-if="key > index">敬请期待</view>
								<view class="em font-size-activity-tag color-sub" :class="seckillId == item.id ? 'color-base-bg active' : ''"
								 v-if="key == index && !item.isNow">
									即将开始
								</view>
								<view class="em font-size-activity-tag color-sub" :class="seckillId == item.id ? 'color-base-bg active' : ''"
								 v-if="key == index && item.isNow">
					...
					<view v-if="!dataList.length && showEmpty" style="z-index: 1 !important;">
						<ns-empty :fixed="!1" text="该时间段暂无秒杀商品"></ns-empty>
					</view>
				</block>
			</mescroll-uni>
		</view>
